"""Index-constituent lists for the 3 universe-bots (see config.UNIVERSE_BOTS).
...
engine/stage1_ranking.py's job, fetched once and shared across all 3 universes.
This module only answers "which symbols belong to universe X" — a cheap,
CSV-based lookup, independent of live market data.

'bot_551' and 'bot_400' are constructed as, and must always remain, SUBSETS of
'bot_751' — this is a structural requirement of Stage 1's design (see
Architecture.md), not just documentation. If this ever needs a 4th universe
whose relationship to bot_751 isn't a clean subset, Stage 1 needs to change too.
"""
...
def _fetch_index_symbols(index_key: str, force_refresh: bool = False) -> list[str]:
    ...
    NOTE (subset-safety, see Architecture.md): each of the 4 indices caches and
    ...
    rebalance than niftytotalmarket's does — see get_universe_symbols's
    subset-safety warning below for how this is handled defensively."""
...
def get_universe_symbols(universe: str, force_refresh: bool = False) -> set[str]:
    """Resolves one of the 3 configured universes to its symbol set. Returns a
    set (not a list) since Stage 1 only ever needs membership tests, and
    set-difference is how bot_551/bot_400 are constructed in the first place."""
    if universe == "total_market":
        return set(_fetch_index_symbols("niftytotalmarket", force_refresh))
    if universe == "total_market_minus_200":
        total = set(_fetch_index_symbols("niftytotalmarket", force_refresh))
...
def filter_to_universe(rank_list: pd.DataFrame, universe: str, top_n: int,
    ...
    pct_change. No network call here — this is why 3 universes only need ONE
    ...
    4 underlying index CSVs refreshed at slightly different times around an NSE
